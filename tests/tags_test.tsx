...
describe("i18n-react", () => {
  ...
  it("renders tag type", () => {
    ...
    expect(renderElement(<T.text text="x" tag="strong"/>)).toBe("<strong>X</strong>");
    expect(renderElement(<T.text text={{ key:"x", tag:"strong" }}/>)).toBe("<strong>X</strong>");
    expect(renderElement(<T.text text={{ key:"y" }} id='B1' tag="strong"/>)).toBe('<strong id="B1">Y</strong>');
  ...
  it("known props are not leaked into DOM", () => {
    ...
    expect(renderElement(<T.text text="x" tag="strong" context="b"/>)).toBe("<strong>Xb</strong>");
    expect(renderElement(<T.text text={{ key:"x", context: 'b' }} tag="p"/>)).toBe("<p>Xb</p>");
